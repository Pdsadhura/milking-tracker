...
router.post("/", async (req, res) => {
  try {
    ...
    if (!start_time || !end_time || duration == null || milk_quantity == null) {
      return res.status(400).json({ status: 400 ,error: "start_time, end_time, duration, milk_quantity are required" });
    }

    if (new Date(end_time) <= new Date(start_time)) {
      return res.status(400).json({  status: 400, error: "end_time must be after start_time" });
    }
    if (duration < 0 || milk_quantity < 0) {
      return res.status(400).json({  status: 400, error: "duration and milk_quantity must be >= 0" });
    }
